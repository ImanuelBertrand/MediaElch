    ...
    QMainWindow(parent), ui(new Ui::MainWindow), m_settings{settings}
{
    ...
    connect(Manager::instance()->tvShowFileSearcher(), &TvShowFileSearcher::tvShowsLoaded, ui->tvShowFilesWidget, [this]() { ui->tvShowFilesWidget->renewModel(true); });
...
void MainWindow::onActionSearch()
{
    ...
    if (current == MainWidgets::Movies) {
        ...
        const elch_ssize_t itemsSelected = ui->tvShowFilesWidget->selectedEpisodes(false).count() + //
